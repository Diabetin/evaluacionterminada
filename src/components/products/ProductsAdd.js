...
const ProductsAdd = () => {
    ...
    const save = async (event) => {
        ...
        var { productName, MSU, price, stock, MDPrice, MDPercentage } = document.forms[0];
        var errorss = "";
        errorss += parseInt(MDPrice.value) > (parseInt(price) / 2) ? "El descuento por precio no puede superar la mitad del unitario.\n" : "";
        errorss += parseInt(MDPercentage.value) > 51 ? "El descuento por descuento no puede superar el 50%.\n" : "";
        if (errorss.length > 0) {
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({
                    "operatorId": config.operatorId, "name": productName.value, "MSU": MSU.value, "price": price.value, "stock": stock.value,
